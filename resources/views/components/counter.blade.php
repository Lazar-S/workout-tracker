@props(["name", "count", "buttonClasses"])
<span class="flex flex-col justify-center items-center flex-1 min-h-17 min-w-6">
    <button data-name="{{ $name }}" data-action="increment" type="submit" class="{{ $buttonClasses }}">
    ...
    <span>{{ $count }}</span>
    <button data-name="{{ $name }}" data-action="decrement" type="submit" class="{{ $buttonClasses }}">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor"
             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
             class="lucide lucide-play-icon lucide-play">
